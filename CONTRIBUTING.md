...
- Review-provider behavior is centralized in `bin/fm-review-provider-lib.sh`.
  Keep GitHub and code.byted.org/GitLab detection and state lookups there, with tests that fake `gh`/`gh-axi` and `bytedcli` instead of requiring live provider access.
...
tests/fm-dashboard.test.sh                # dashboard snapshot/server/focus behavior, in-flight summaries, and Age fallback tests
...
tests/fm-review-provider.test.sh          # review-provider detection and fake GitHub/code.byted.org state lookup tests
tests/fm-teardown.test.sh                 # fm-teardown.sh landed-work safety and reminder checks: fork-remote allow, squash/content landings, dirty and unlanded refusals, PR/MR-head metadata, no-pr= branch discovery, tasks-axi/manual backlog reminder, --force override, stale-vs-live worktree git index.lock recovery
