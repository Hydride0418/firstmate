...
fm_backend_tmux_create_task() {  # <session> <window-name> <proj-abs>
  local ses=$1 wname=$2 proj_abs=$3 t
  if tmux list-windows -t "$ses" -F '#{window_name}' | grep -qx "$wname"; then
    echo "error: window $ses:$wname already exists" >&2
    return 1
  fi
  tmux new-window -d -t "$ses" -n "$wname" -c "$proj_abs"
  # Keep the window name anchored to the task id: some harnesses rename the window
  # or set a terminal title on startup. Disabling automatic/allow rename makes the
  # anchor persistent, and seeding the pane title helps UIs that read it early. A
  # running application can still update pane_title later.
  t="$ses:$wname"
  ...
  tmux rename-window -t "$t" "$wname" >/dev/null 2>&1 || true
  tmux select-pane -t "$t" -T "$wname" >/dev/null 2>&1 || true
}
