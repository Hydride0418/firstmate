...
# Re-anchor the tmux window name to the task id after a lifecycle step that may
# drift it (treehouse get, the launch command). No-op for non-tmux backends, which
# manage their own tab labels; fm_backend_tmux_create_task sets the same persistent
# options at window creation, so this is belt-and-suspenders against title drift.
pin_task_title_tmux() {
