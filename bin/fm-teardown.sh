...
# clear volatile state, refresh/prune the project's clone for PR/MR-based ship
...
# normal ship task whose commits are not so reachable - when its PR/MR is merged and
# the review provider reports a review head that contains the current local work, or its
...
# no pr= was ever recorded (e.g. a yolo-authorized merge on a repo with no PR/MR CI,
# where the usual "checks green" fm-pr-check.sh trigger never fires) - by looking
# up a merged review whose head branch matches the worktree's branch, fetching its head
# via refs/pull/<n>/head (GitHub) when the branch itself was deleted. So a missing pr= never
...
# Is the worktree's review merged for local work contained in it? Resolves the
# review from the recorded pr= URL first, then from the branch name, and asks the
# provider for state and head. GitHub PRs use robust containment (exact head,
# ancestor, or patch-id match, fetching refs/pull/<n>/head when the branch was
# deleted); code.byted.org MRs use the conservative provider check (merged state
# and exact head). Returns non-zero when not merged, the current work is not
# contained, no review is found, or any provider error occurs - the caller then
...
review_is_merged() {
  ...
  [ -n "$target" ] || return 1
  provider=$(fm_review_provider_for_target "$target" "$WT" 2>/dev/null || true)
  if [ "$provider" = gitlab ]; then
    # code.byted.org MRs: conservative provider check (merged state + exact head).
    fm_review_is_merged_for_head "$target" "$WT"
    return
  fi
  # GitHub PRs: robust containment - resolve the merged head, fetch it via
  # refs/pull/<n>/head when the branch was deleted, and accept when the current
  # work is the head, an ancestor of it, or patch-contained in it.
...
# current local work is contained in the review head, OR the content is already in
# the default branch (fallback, which also covers the no-review and provider-error
