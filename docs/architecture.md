...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR/MR merge polling or an X mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, and heartbeat backstop hits.
...
`fm-home-seed.sh` provisions the isolated home, clones the listed review-based projects into it, initializes newly cloned `no-mistakes` projects, copies the charter to `data/charter.md`, and `fm-spawn.sh --secondmate` launches it through the same session-provider and status-file path as any direct report.
...
`local-only` projects stay with the main first mate because they merge into the main local checkout instead of a remote-backed PR/MR path.
...
`no-mistakes` projects run the full validation pipeline, `direct-PR` projects open PRs/MRs without that pipeline, and `local-only` projects stay local until firstmate performs an approved fast-forward merge.
Review-based work targets either GitHub pull requests or code.byted.org GitLab merge requests; `bin/fm-review-provider-lib.sh` detects the provider from the review URL or remote and looks up review state and head SHA through `gh`/`gh-axi` or `bytedcli codebase`, so PR-check arming and teardown stay provider-agnostic.
...
PR-based task merges (GitHub) go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
...
Landed work is accepted when `HEAD` is reachable from any remote-tracking branch, when a merged PR/MR's provider head contains the current local work, or when the worktree content is already present in the freshly fetched default branch.
...
Those PR-head and content checks let a squash-merged PR/MR whose head branch was deleted tear down cleanly without using `--force`; `local-only` work instead tears down after the approved local default-branch merge or after the branch is pushed to any remote.
...
The locked session-start bootstrap step, review-based teardown, and merged-PR/MR wake handling refresh remote-backed project clones when the clone is safe to move.
