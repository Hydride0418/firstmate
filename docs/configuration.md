...
FM_DASHBOARD_RUNTIME_DIR=$FM_HOME/.lavish/fm-dashboard   # optional pid/log dir for bin/fm-dashboard.sh serve
...
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|PR ready|MR ready|checks green|ready in branch|merged'   # status regex that makes watcher and daemon signal/stale/scan output captain-relevant
