...
# and the review provider reports a review head that contains the current local work, or its
# content is already in the up-to-date default branch.
#
# Covers three fixes:
...
#   (r) fm-pr-check with code.byted.org MR                      -> records pr_head and check wakes on merge
#   (s) no-mistakes + merged code.byted.org MR                  -> ALLOW  (GitLab provider path)
#
...
test_gitlab_pr_check_records_head_and_check_script() {
  ...
  grep -qxF "pr_head=$head" "$case_dir/state/task-x1.meta" \
    || fail "gitlab-pr-check: MR head was not recorded in meta"
